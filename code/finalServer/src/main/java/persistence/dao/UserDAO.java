...
public class UserDAO {
    ...
    public void updateInfirmaryPK(UserDTO userDTO)
    {
        SqlSession session = sqlSessionFactory.openSession();
        Map<String, Integer> map = new HashMap<>();
        map.put("pk", userDTO.getPk());
        map.put("infirmary_pk", userDTO.getInfirmary_pk());

        try {
            session.update("mapper.UserMapper.updateInfirmaryPK", map);
            session.commit();
        } catch(Exception e) {
            System.out.println(e);
            session.rollback();
    ...
    public int updateUserInfo(UserDTO userDTO) {
        ...
        int row = 0;
        try {
            row =  session.update("mapper.UserMapper.updateUserInfo", userDTO);
            session.commit();
        } catch (Exception e) {
            session.rollback();
        } finally {
